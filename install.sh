...
main() (
    ...
    printf '%s\n' 'Execute no seu terminal:' '  export PATH="$HOME/.local/bin:$PATH"' '  musaranho token create --name teste' '  musaranho serve' '' 'Swagger: http://127.0.0.1:8888/docs' 'Modelo musaranho-0.1 instalado para inferência local em CPU.'
    exit 0
)
