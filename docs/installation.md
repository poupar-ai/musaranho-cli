...
O pacote é `cli/musaranho-0.1.0-x86_64-unknown-linux-gnu.tar.gz`, acompanhado de `cli/SHA256SUMS`. Para executar sem instalar:

```bash
(cd cli && sha256sum -c SHA256SUMS)
MUSARANHO_TEST_DIR="$(mktemp -d)"
tar -xzf cli/musaranho-0.1.0-x86_64-unknown-linux-gnu.tar.gz -C "$MUSARANHO_TEST_DIR"
cd "$MUSARANHO_TEST_DIR/musaranho-0.1.0-x86_64-unknown-linux-gnu"
./musaranho --version
```
